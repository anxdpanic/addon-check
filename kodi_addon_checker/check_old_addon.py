...
def check_for_existing_addon(report: Report, addon_path: str, all_repo_addons: dict, pull_request: bool,
    ...
    for branch, repo in sorted(all_repo_addons.items(), reverse=True):
        if KodiVersion(branch) <= kodi_version and addon_name in repo:
            _check_versions(report, addon_details, branch, repo.find(addon_name).version, pull_request)
            return
...
def _check_versions(report: Report, addon_details, branch, repo_addons_version, pull_request):
    """Check for version bump in the existing addon
        ...
        :pull_request:        boolean value indicating whether the check is
